...
<html>
    ...
    <a href="<?= HOST ?>oplossing-file-upload-artikel-toevoegen-form.php">Voeg een artikel toe</a>
    ...
    <?php if ( $artikels ): ?>
		...
		<?php foreach ($artikels as $artikel): ?>
			...
			<article class="<?= ( $artikel[ 'is_active' ] == 0 ) ? 'non-active' : '' ?>">
                ...
                <a href="<?= HOST ?>oplossing-file-upload-artikel-wijzigen-form.php?edit=<?= $artikel['id'] ?>">artikel wijzigen</a> | 
                	<form class="inline" action="<?= HOST ?>oplossing-file-upload-artikel-activeren-process.php" method="POST">
                		<button name="activate" value="<?= $artikel['id']?>">artikel <?= ( $artikel[ 'is_active' ] == 0 ) ? '' : 'de' ?>activeren</button> 
                	</form> |
                	<form class="inline" action="<?= HOST ?>oplossing-file-upload-artikel-delete-process.php" method="POST">
